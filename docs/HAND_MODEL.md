...
- Commands use a damped second-order filter, bounded to 120°/s and 600°/s² before joint-limit clamping. Actual angles result from the solver and may differ under load or contact. An optional wrist–finger target coupling illustrates tenodesis direction; it is off by default and does not model passive tissue forces. See [research and implementation assumptions](SIMULATION_REFINEMENT.md).
...
- The render accumulator accepts at most 0.1 s per frame; excess frame time is discarded and reported in the angle inspector. Hidden browser documents and inactive workspace tabs skip physics and rendering. Poses interpolate between solved steps for smoother display; paused views show the current solved pose. Physics time can lag wall time and is separate from the acquisition/replay clock.
